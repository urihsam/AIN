...
data = dataset(FLAGS.DATA_DIR, split_ratio=0.8, normalize=FLAGS.NORMALIZE, biased=FLAGS.BIASED, 
...
if FLAGS.IS_TARGETED_ATTACK:
    np.save("fgsm_tiny_tgt_ys_diversity.npy", ys)
    np.save("fgsm_tiny_tgt_diversity.npy", diff)
else:
    np.save("fgsm_tiny_untgt_ys_diversity.npy", ys)
    np.save("fgsm_tiny_untgt_diversity.npy", diff)
