        ...
        with g.as_default():
            ...
            for i in range(num_gpus):
                with tf.device(assign_to_device('/gpu:{}'.format(i), ps_device='/cpu:0')):
                    ...
                    _img = images_holder[i * FLAGS.BATCH_SIZE: (i+1) * FLAGS.BATCH_SIZE]
                    _label = label_holder[i * FLAGS.BATCH_SIZE: (i+1) * FLAGS.BATCH_SIZE]
                    # model
                    model = attain.ATTAIN(_img, _label, low_bound_holder, up_bound_holder, epsilon_holder, is_training)
