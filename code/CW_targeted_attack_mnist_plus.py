...
with tf.Session(config=config, graph=g) as sess:
    ...
    for epoch in range(1):
        ...
        for class_id in range(FLAGS.NUM_CLASSES):
            ...
            atk_path = clean_path + "/cw_t{}".format(targeted_class_id)
            ...
            for clean_img_name in clean_images_names[:100]:
                ...
                if not os.path.exists(atk_img_name):
                    ...
                    atk_data = cw.attack(sess, clean_img, targeted_label)
        ...
        for class_id in range(FLAGS.NUM_CLASSES):
            ...
            clean_images_names = glob.glob(os.path.join(clean_path, '*.npy'))
            np.random.shuffle(clean_images_names)
            for clean_img_name in clean_images_names[:10]:
                path_name = clean_img_name.split("/")
                atk_img_name = os.path.join(atk_path, path_name[-1])
                if not os.path.exists(atk_img_name):
                    test_count += 1
